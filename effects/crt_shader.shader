...
// Taken from https://github.com/AaronWizard/CRTScreenShader
...
void fragment()
{
	...
	vec2 input_size = vec2(textureSize(TEXTURE, 0));
	...
	col  = filter_lanczos(coeffs, get_color_matrix(TEXTURE, tex_co, dx));
	col2 = filter_lanczos(coeffs, get_color_matrix(TEXTURE, tex_co + dy, dx));
