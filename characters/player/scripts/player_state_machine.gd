...
func _physics_process(delta: float) -> void:
	if owner.is_controllable:
		input_direction = Vector2(int(Input.is_action_pressed("action_right")) - int(Input.is_action_pressed("action_left")),
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("action_jump") and Input.is_action_pressed("action_down"):
	...
	if event.is_action_pressed("action_jump") and \
			not Input.is_action_pressed("action_up") and \
			not Input.is_action_pressed("action_down"):
